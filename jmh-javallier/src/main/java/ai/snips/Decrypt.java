...
public class Decrypt {
    ...
    public void testDecrypt_small_1024(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_1024, state.cipher_small_1024);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_small_2048(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_2048, state.cipher_small_2048);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_small_3072(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_3072, state.cipher_small_3072);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_small_4096(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_4096, state.cipher_small_4096);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_large_1024(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_1024, state.cipher_large_1024);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_large_2048(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_2048, state.cipher_large_2048);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_large_3072(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_3072, state.cipher_large_3072);
    }

    @Benchmark()
    @BenchmarkMode(Mode.AverageTime)
    @OutputTimeUnit(TimeUnit.MILLISECONDS)
    public void testDecrypt_large_4096(Blackhole blackhole, Keys state) {
        decrypt(blackhole, state.key_4096, state.cipher_large_4096);
    }

    private static void decrypt(Blackhole blackhole, PaillierPrivateKey key, EncryptedNumber message) {
        blackhole.consume(key.decrypt(message));
    }
